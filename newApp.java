...
public class newApp extends Application {
    ...
    private void setupUI(Stage primaryStage) {
        ...
        Scene scene = new Scene(root, 1115, 675);
    ...
    private void showGameDetails(Game game) {
        ...
        // Yeni: Bilgi ve görsel için GridPane
        ...
        try {
            ...
            if (imagePath == null || imagePath.isEmpty() || imagePath.equals("No image selected")) {
                imagePath = "images/default_game.png";
            }
            Image image;
            if (imagePath.startsWith("http")) {
                image = new Image(imagePath, 220, 160, true, true);
            } else {
                image = new Image("file:" + imagePath, 220, 160, true, true);
            }
            imageView.setImage(image);
        } catch (Exception e) {
            imageView.setImage(new Image("file:images/default_game.png", 220, 160, true, true));
        }
        imageView.setFitWidth(220);
        imageView.setPreserveRatio(true);
        imageView.setSmooth(true);
        imageView.setCache(true);
        imageView.setStyle("-fx-effect: dropshadow(gaussian, #888, 8, 0.2, 2, 2);"
            + "-fx-border-color: #bbb; -fx-border-width: 2; -fx-background-color: #fff;");
...
GridPane.setValignment(imageView, VPos.TOP);      // Görseli üstte hizala
GridPane.setHalignment(imageView, HPos.RIGHT);    // Görseli sağa hizala
GridPane.setValignment(infoGrid, VPos.TOP);       // Bilgileri üstte hizala
// ...existing code...
        ...
        root.getChildren().addAll(
            ...
            ratingBox,
            separator1,
            topGrid, // Burada topBox yerine topGrid kullanıyoruz
            separator2,
            categoriesBox,
            closeButton,
            moreInfoLink
...
Image image;
if (imagePath.startsWith("http")) {
    image = new Image(imagePath, 100, 100, true, true);
} else {
    image = new Image("file:" + imagePath, 100, 100, true, true);
}
imageView.setImage(image);
                } catch (Exception e) {
                    // HATA OLURSA BURADA DEFAULT GÖRSELİ GÖSTER
                    imageView.setImage(new Image("file:images/default_game.png", 100, 100, true, true));
                }
                ...
                imageView.setStyle("-fx-effect: dropshadow(gaussian, #888, 8, 0.2, 2, 2);"
                    ...
                    container.getChildren().addAll(imageView,titleBox, detailsLabel, genresLabel, platformsLabel);
    ...
    private void showAddGameDialog() {
        ...
        developerField.setMinWidth(250);
        publisherField.setMinWidth(250);
        genreField.setMinWidth(250);
        platformField.setMinWidth(250);
        steamIdField.setMinWidth(250);
        releaseYearField.setMinWidth(250);
        playtimeField.setMinWidth(250);
        tagsField.setMinWidth(250);
        ratingField.setMinWidth(250);
        ...
        addImageButton.setOnAction(e -> {
            ...
            File selectedFile = imageChooser.showOpenDialog(dialog.getDialogPane().getScene().getWindow());
            if (selectedFile != null) {
                try {
                    // Resmi images klasörüne kopyala
                    String newImagePath = IMAGE_DIR + selectedFile.getName();
                    Files.copy(selectedFile.toPath(), 
                        Paths.get(newImagePath), 
                        StandardCopyOption.REPLACE_EXISTING);
                    imagePathLabel.setText(newImagePath);
                } catch (IOException ex) {
                    imagePathLabel.setText("Error copying image");
                }
            }
        });
            ...
            grid.add(addImageButton, 0, 10);
            grid.add(imagePathLabel, 1, 11);
            ...
            dialog.setResultConverter(buttonType -> {
                if (buttonType == ButtonType.OK) {
                    try {
                        ...
                        String steamId = String.valueOf(Integer.parseInt(steamIdText));
                        String releaseYear =String.valueOf(Integer.parseInt(releaseYearText));
                        String playtime = String.valueOf(Integer.parseInt(playtimeText));
                        String rating = String.valueOf(Double.parseDouble(ratingText));
    ...
    imagePathLabel.getText() // <-- Eksikse ekleyin!
                        ...
                        alert.setHeaderText("Please enter valid numbers for Steam ID, Release Year, Playtime, and Rating.");
                        alert.showAndWait();
...
Button addImageButton = new Button("Change Image");
        Button setDefaultImageButton = new Button("Use default image "); // yeni buton
        Label imagePathLabel = new Label(selectedGame.getImagePath() != null ? selectedGame.getImagePath() : "No image selected");
        addImageButton.setOnAction(e -> {
            FileChooser imageChooser = new FileChooser();
            imageChooser.getExtensionFilters().addAll(
                new FileChooser.ExtensionFilter("Image Files", "*.png", "*.jpg", "*.jpeg", "*.svg")
        ...
        //merhaba
        setDefaultImageButton.setOnAction(e -> {
            imagePathLabel.setText("images/default_game.png");
        });
            ...
            grid.add(new Label("Rating:"), 0, 9);
            grid.add(ratingField, 1, 9);
            grid.add(new Label("Image Path:"), 0, 10);
        HBox imageButtonBox = new HBox(10, addImageButton, setDefaultImageButton);
        grid.add(imageButtonBox, 1, 10);
        grid.add(imagePathLabel, 1, 11);
            ...
            dialog.setResultConverter(buttonType -> {
                if (buttonType == ButtonType.OK) {
                    try {
                        // Validate numeric inputs
                        int steamId = Integer.parseInt(steamIdField.getText());
                        int releaseYear = Integer.parseInt(releaseYearField.getText());
                        int playtime = Integer.parseInt(playtimeField.getText());
                        double rating = Double.parseDouble(ratingField.getText());

                        String imagePath = imagePathLabel.getText();
                if (imagePath.equals("No image selected") || imagePath.isEmpty()) {
                    imagePath = "images/default_game.png";
                }

                        // If parsing is successful, create and return the Game object
                        return new Game(
                                titleField.getText(),
                                developerField.getText(),
                                publisherField.getText(),
